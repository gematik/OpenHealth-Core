...
import de.gematik.openhealth.crypto.internal.interop.AesCipher as JniAesCipher
...
private class JvmAesCipher(
    ...
    private val cipher by lazyDeferred<JniAesCipher> {
        deferScoped(allowReturnClassHandle = true) {
            val cipher =
                JniAesCipher.createEncryptor(
    ...
    override fun update(data: ByteArray): ByteArray =
        runWithProvider {
            cipher.update(data.toUint8Vector().alsoDefer()).toByteArray()
        }

    override fun final(): ByteArray =
        runWithProvider {
            cipher._final().toByteArray()
        }

    override fun authTag(): ByteArray =
        runWithProvider {
            if (spec is AesGcmCipherSpec) {
                cipher.getAuthTag(spec.tagLength.bytes.toLong()).toByteArray()
            } else {
                byteArrayOf()
            }
        }
...
/**
 * Creates a new [AesCipher] instance for the given [CryptoScope] and [SecretKey].
 */
internal actual fun AesCipherSpec.nativeCreateCipher(
    scope: CryptoScope,
    key: SecretKey,
): AesCipher = JvmAesCipher(this, scope, key)

/**
 * Creates a new [AesDecipher] instance for the given [CryptoScope] and [SecretKey].
 */
internal actual fun AesDecipherSpec.nativeCreateDecipher(
    scope: CryptoScope,
    key: SecretKey,
): AesDecipher = JvmAesDecipher(this, scope, key)
