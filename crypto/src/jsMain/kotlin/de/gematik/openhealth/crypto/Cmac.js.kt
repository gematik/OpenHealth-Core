...
/**
 * JavaScript implementation of the CMAC algorithm.
 */
private class JsCmac(
    ...
    private val cmac by lazyDeferred {
        Cmac.create(secret.data.toUint8Vector().alsoDefer(), "AES-${secret.length.bits}-CBC")
    }
    ...
    override fun final(): ByteArray =
        runWithProvider {
            cmac.final().alsoDefer().toByteArray()
        }
}

/**
 * JavaScript-specific implementation for creating CMAC instances.
 */
