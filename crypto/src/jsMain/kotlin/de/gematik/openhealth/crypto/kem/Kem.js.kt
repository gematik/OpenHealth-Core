...
private class JsKemEncapsulation(
    ...
    scope: CryptoScope
) : KemEncapsulation, DeferScope by deferred(scope) {
    private val kem by lazyDeferred {
        deferScoped(true) {
            MlKemEncapsulation.create(spec.algorithm.algorithmName(), encapsulationKey.toUint8Vector().alsoDefer())
        }
    }
        ...
        runWithProvider {
            deferScoped {
                ...
                if (spec.algorithm.isKyber()) {
                    sharedSecret = kyberSharedSecret(encapsulationKey.toUint8Vector().alsoDefer(), sharedSecret)
                }
...
private class JsKemDecapsulation(
    override val spec: KemSpec,
    scope: CryptoScope
) : KemDecapsulation, DeferScope by deferred(scope) {
    private val kem by lazyDeferred {
    ...
    override fun decapsulate(wrappedKey: ByteArray): KemDecapsulationResult =
        runWithProvider {
            deferScoped {
                ...
                if (spec.algorithm.isKyber()) {
                    sharedSecret = kyberSharedSecret(kem.getEncapsulationKey().alsoDefer(), sharedSecret)
                }
...
private fun DeferScope.kyberSharedSecret(encapsulationKey: Uint8Vector, sharedSecret: Uint8Vector) = runWithProvider {
