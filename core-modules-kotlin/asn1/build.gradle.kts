...
// Location where UniFFI drops generated Kotlin code and native libs.
...
android {
    namespace = "de.gematik.openhealth.asn1"
    compileSdk = 36
    defaultConfig {
        minSdk = 24
...
tasks.matching { it.name == "compileTestKotlinJvm" || it.name == "jvmTest" }.configureEach {
    onlyIf { file(generatedKotlinDir).exists() }
}
...
mavenPublishing {
    ...
    coordinates(artifactId = "asn1")

    pom {
        name = "OpenHealth ASN.1"
        description = "OpenHealth ASN.1 utilities (KMP) backed by Rust + UniFFI"
        inceptionYear = "2026"
