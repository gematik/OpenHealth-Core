...
data class Asn1UtcTime(
     ...
     */
    data class Offset(val hours: Int, val minutes: Int)
}

private val utcTimeRegex = Regex("""(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})(\d{2})?(Z|[+-]\d{2}\d{2})""")
...
fun Asn1Decoder.ParserScope.readUtcTime(): Asn1UtcTime =
    advanceWithTag(Asn1Type.UtcTime) {
        ...
        val (yy, mm, dd, hh, min, ss, offset) = match?.destructured ?: fail { "Wrong utc time format: `$value`" }
...
fun Asn1Encoder.WriterScope.writeUtcTime(value: Asn1UtcTime) {
    writeTaggedObject(Asn1Type.UtcTime) {
        val formattedTime = buildString {
            append(value.year.rem(100).toString().padStart(2, '0'))
...
data class Asn1GeneralizedTime(
     ...
     * Optional offset from UTC. [hours] can be negative.
     */
    class Offset(val hours: Int, val minutes: Int)
}

private val generalizedTimeRegex = Regex("""(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})?(\d{2})?(\.\d{1,3})?(Z|[+-]\d{2}\d{2})?""")
...
fun Asn1Decoder.ParserScope.readGeneralizedTime(): Asn1GeneralizedTime =
    advanceWithTag(Asn1Type.GeneralizedTime) {
        ...
        val (yyyy, mm, dd, hh, min, ss, fff, offset) = match?.destructured ?: fail { "Wrong generalized time format: `$value`" }
