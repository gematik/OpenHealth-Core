...
class PaceKeyTest {
    ...
    fun testEqualsAndHashCode() =
        runTestWithProvider {
            val key1 = getAES128Key(secretK, Mode.ENC)
            val key2 = getAES128Key(secretK, Mode.MAC)

            val paceKey1 = PaceKey(SecretKey(key1), SecretKey(key2))
            val paceKey2 = PaceKey(SecretKey(key1), SecretKey(key2))
            val paceKey3 = PaceKey(SecretKey(key2), SecretKey(key1))

            assertEquals(paceKey1, paceKey2)
            assertNotEquals(paceKey1, paceKey3)
            assertNotEquals<PaceKey?>(paceKey1, null)
            assertEquals(paceKey1.hashCode(), paceKey2.hashCode())
            assertNotEquals(paceKey1.hashCode(), paceKey3.hashCode())
        }
}
