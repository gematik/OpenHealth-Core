...
impl HashGenerator {
    ...
    pub fn new(hash_name: &str) -> Result<Self, OsslError> {
        ...
        if md.is_null() {
            return Err(throw_openssl_error(
        ...
        if ctx.is_null() {
            ...
            return Err(throw_openssl_error("Failed to create EVP_MD_CTX"));
        }
        Ok(HashGenerator { md, ctx, output_length: 0 })
    }

    /// Convenience constructor that also calls DigestInit.
    pub fn create(hash_name: &str) -> Result<Self, OsslError> {
