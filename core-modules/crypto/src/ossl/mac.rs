...
impl Mac {
    ...
    pub fn create(key: &[u8], name: &str, cipher: Option<&str>, digest: Option<&str>) -> OsslResult<Self> {
        ...
        if let Some(val_cipher) = &val_cipher {
            params.push(unsafe {OSSL_PARAM_construct_utf8_string(name_cipher.as_ptr(), val_cipher.as_ptr() as *mut _, 0)});
        }

        if let Some(val_digest) = &val_digest {
            params.push(unsafe {OSSL_PARAM_construct_utf8_string(name_digest.as_ptr(), val_digest.as_ptr() as *mut _, 0)});
        }
