...
pub struct CVCertPublicKey {
    /// Raw contents of the public key container (tag 7F49).
...
fn parse_certificate_body(scope: &mut ParserScope) -> Result<CertificateBody, Asn1DecoderError> {
    scope.advance_with_tag(TAG_CERTIFICATE_BODY.application_tag().constructed(), |scope| {
        ...
        let certificate_holder_reference =
            read_application_bytes(scope, TAG_CERTIFICATE_HOLDER_REFERENCE, 1, 16)?;
        ...
        let certificate_extensions = if scope.remaining_length() > 0 {
            Some(parse_certificate_extensions(scope)?)
        } else {
            None
        };
...
fn read_profile_identifier(scope: &mut ParserScope) -> Result<u8, Asn1DecoderError> {
    scope.advance_with_tag(TAG_PROFILE_IDENTIFIER.application_tag().primitive(), |scope| {
        let bytes = scope.read_bytes(scope.remaining_length())?;
        if bytes.is_empty() {
            return Err(Asn1DecoderError::custom("CertificateProfileIdentifier must not be empty"));
        }
        if bytes.len() > 4 {
            return Err(Asn1DecoderError::custom(
                "CertificateProfileIdentifier must not exceed 4 bytes",
            ));
        }
        if (bytes[0] & 0x80) != 0 {
            return Err(Asn1DecoderError::custom(
                "CertificateProfileIdentifier must be a non-negative INTEGER",
            ));
        }
        let value = bytes.iter().fold(0u32, |acc, b| (acc << 8) | (*b as u32));
        if value > u8::MAX as u32 {
            return Err(Asn1DecoderError::custom(
                "CertificateProfileIdentifier must be within 0..=255",
            ));
        }
        Ok(value as u8)
    })
...
fn parse_public_key(scope: &mut ParserScope) -> Result<CVCertPublicKey, Asn1DecoderError> {
    scope.advance_with_tag(TAG_PUBLIC_KEY.application_tag().constructed(), |scope| {
        let data = scope.read_bytes(scope.remaining_length())?;
        if data.is_empty() {
            return Err(Asn1DecoderError::custom("CVCertPublicKey must not be empty"));
        }
        Ok(CVCertPublicKey { public_key_data: data })
    })
}
...
fn parse_date(scope: &mut ParserScope, tag_number: u32) -> Result<CertificateDate, Asn1DecoderError> {
    scope.advance_with_tag(tag_number.application_tag().primitive(), |scope| {
        let bytes = scope.read_bytes(scope.remaining_length())?;
        if bytes.len() != 6 {
            return Err(Asn1DecoderError::custom("Certificate date must be 6 bytes"));
        }
        for (idx, byte) in bytes.iter().enumerate() {
            if *byte > 9 {
                return Err(Asn1DecoderError::custom(format!(
                    "Certificate date digit {idx} must be 0..9"
                )));
            }
        }
...
fn parse_extension_field(scope: &mut ParserScope) -> Result<ExtensionField, Asn1DecoderError> {
    ...
    let value = match length {
        ...
        Asn1Length::Indefinite => {
            return Err(Asn1DecoderError::custom(
                "Indefinite length is not supported for extension fields",
            ))
        }
    };
...
fn read_application_bytes(
    ...
    scope.advance_with_tag(tag_number.application_tag().primitive(), |scope| {
        ...
        if len < min_len || len > max_len {
            return Err(Asn1DecoderError::custom(format!(
                "Invalid length {len} for application tag {tag_number}"
            )));
        }
...
mod tests {
    ...
    fn build_test_certificate(with_extensions: bool) -> Vec<u8> {
        Asn1Encoder::write::<crate::error::Asn1EncoderError>(|w| {
            w.write_tagged_object(
                TAG_CV_CERTIFICATE.application_tag().constructed(),
                |cert| -> EncResult {
                    cert.write_tagged_object(
                        TAG_CERTIFICATE_BODY.application_tag().constructed(),
                        |body| -> EncResult {
                    ...
                    body.write_tagged_object(TAG_CERTIFICATE_HOLDER_REFERENCE.application_tag(), |field| -> EncResult {
                    ...
                    body.write_tagged_object(TAG_CERTIFICATE_EXPIRATION_DATE.application_tag(), |field| -> EncResult {
    ...
    fn parse_cv_certificate_without_extensions() {
        ...
        assert_eq!(cert.body.public_key.public_key_data, vec![0xA0, 0x01, 0x02]);
    ...
    fn reject_invalid_date_digit() {
        let data =
            build_certificate_with_fields(&[0x00], b"CAR", b"CHR", &[2, 5, 10, 1, 0, 1], &[2, 5, 0, 1, 0, 2]);
        assert!(CVCertificate::parse(&data).is_err());
    }
    ...
    fn reject_invalid_month() {
        let data =
            build_certificate_with_fields(&[0x00], b"CAR", b"CHR", &[2, 5, 1, 3, 0, 1], &[2, 5, 0, 1, 0, 2]);
        assert!(CVCertificate::parse(&data).is_err());
    }
